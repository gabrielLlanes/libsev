...
try(Arena arena = Arena.ofConfined()) {
  ...
  int server = Socket.socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
  Socket.setSockOpt(server, SOL_SOCKET, SO_REUSEADDR, 1);
  ...
  Socket.bind(server, localhost, SOCKADDR_IN_SIZE);
  Socket.listen(server, 1);

  UringLoop loop = UringLoop.init();
  Operation acceptOperation = new Operation.Accept();
  acceptOperation.fd(server);
...
## Building
Run `./gradlew assemble`. The jar file in `build/libs` can be included on the classpath and the shared object file in `build/libs/sev/shared` can be added via the option `-Djava.library.path`
